...
/**
 * Returns true if the value is a string, otherwise returns false. return typeof value === 'string' ? true : false
 *
...
function isPalindrome(str) {
  const replasedStr = str.toLowerCase().replace(/[^a-z0-9]/g, '');
  const reverseStr = replasedStr.split('').reverse().join('');
  return reverseStr === replasedStr;
}
...
function findLongestWord(sentence) {
  ...
  let word = arrSentence[0];
  for (let i = 0; i < arrSentence.length; i += 1) {
    if (arrSentence[i].length > word.length) {
      word = arrSentence[i];
    }
  }
  return word;
